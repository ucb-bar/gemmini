...
class LoopMatmulLdD(block_size: Int, coreMaxAddrBits: Int, iterator_bitwidth: Int, max_acc_addr: Int, input_w: Int,
  ...
  val max_blocks = 1.U //Mux(req.low_d, Mux(req.max_j <= max_block_len.U, req.max_j, max_block_len.U),
  ...
  io.loop_id := req.loop_id
  val j_iter_size = Mux(req.num_subarray > 0.U, max_blocks * req.num_subarray, max_blocks)
  ...
  when (req.dram_addr === 0.U) {
    ...
    val next_j = floorAdd(j, j_iter_size, req.max_j, next_i === 0.U)
...
class LoopMatmul(block_size: Int, coreMaxAddrBits: Int, rob_size: Int, max_lds: Int, max_exs: Int, max_sts: Int,
  ...
  when(cmd.valid && is_loop_cmd && !loop_being_configured.configured) {
    ...
    switch (cmd.bits.inst.funct) {
      ...
      is (LOOP_WS) {
        ...
        loop_being_configured.a_ex_spad_id := cmd.bits.rs1(19, 18)
        loop_being_configured.b_ex_spad_id := cmd.bits.rs1(17, 16) // TODO magic numbers
  ...
  ldD.io.req.bits.num_subarray := loop_requesting_ldD.num_subarray
  ...
  stC.io.req.bits.num_subarray := loop_requesting_st.num_subarray

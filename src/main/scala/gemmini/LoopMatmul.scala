...
class LoopMatmulExecuteReq(val block_size: Int, val coreMaxAddrBits: Int, val iterator_bitwidth: Int, val max_addr: Int, val max_acc_addr: Int, val concurrent_loops: Int) extends Bundle {
  ...
  val max_k = UInt(iterator_bitwidth.W)
  val max_i = UInt(iterator_bitwidth.W)
  val pad_j = UInt(log2Up(2*block_size).W)
  val pad_k = UInt(log2Up(block_size).W)
  val pad_i = UInt(log2Up(2*block_size).W)
  val a_tranpose = Bool()
  val b_tranpose = Bool()
  val accumulate = Bool()
...
class LoopMatmul(block_size: Int, coreMaxAddrBits: Int, reservation_station_size: Int, max_lds: Int, max_exs: Int, max_sts: Int,
  ...
  val max_block_len_ij = (dma_max_bytes / (2 * block_size * input_w / 8)) max 1
  val max_block_len_acc_nonfp8 = (dma_max_bytes / (2 * block_size * acc_w / 8)) max 1
  ...
  when(cmd.valid && is_loop_cmd && !loop_being_configured.configured) {
    ...
    switch (cmd.bits.cmd.inst.funct) {
      is (LOOP_WS_CONFIG_BOUNDS) {
        ...
        loop_being_configured.max_i := cmd.bits.cmd.rs2(iterator_bitwidth-1, 0) / divisor

        loop_being_configured.pad_k := cmd.bits.cmd.rs1(iterator_bitwidth * 3 - 1, iterator_bitwidth * 2)
        loop_being_configured.pad_j := cmd.bits.cmd.rs1(iterator_bitwidth * 2 - 1, iterator_bitwidth) / divisor
        loop_being_configured.pad_i := cmd.bits.cmd.rs1(iterator_bitwidth-1, 0) / divisor
      }
      ...
      is (LOOP_WS) {
        ...
        loop_being_configured.narrow_type := (io.activation_mx_format =/= 0.U) && (io.weight_mx_format =/= 0.U)
  ...
  val pad_i = Mux(loop_requesting_st.pad_i/2.U > 1.U, loop_requesting_st.pad_i/2.U, 1.U)
  val pad_j = Mux(loop_requesting_st.pad_j/2.U > 1.U, loop_requesting_st.pad_j/2.U, 1.U)
  val max_i = Mux(loop_requesting_st.max_i/2.U > 1.U, loop_requesting_st.max_i/2.U, 1.U)
  val max_j = Mux(loop_requesting_st.max_j/2.U > 1.U, loop_requesting_st.max_j/2.U, 1.U)
  stC.io.req.bits.max_k := Mux(is_resadd, 1.U, loop_requesting_st.max_k)
  stC.io.req.bits.max_j := Mux(io.activation_mx_format === 0.U, max_j, loop_requesting_st.max_j)
  stC.io.req.bits.max_i := Mux(io.activation_mx_format === 0.U, max_i, loop_requesting_st.max_i)
  stC.io.req.bits.pad_j := Mux(io.activation_mx_format === 0.U, pad_j, loop_requesting_st.pad_j)
  stC.io.req.bits.pad_i := Mux(io.activation_mx_format === 0.U, pad_i, loop_requesting_st.pad_i)
  ...
  stC_spad.io.req.bits.max_j := Mux(io.activation_mx_format === 0.U, max_j, loop_requesting_st.max_j)
  stC_spad.io.req.bits.max_i := Mux(io.activation_mx_format === 0.U, max_i, loop_requesting_st.max_i)
  stC_spad.io.req.bits.pad_j := Mux(io.activation_mx_format === 0.U, pad_j, loop_requesting_st.pad_j)
  stC_spad.io.req.bits.pad_i := Mux(io.activation_mx_format === 0.U, pad_i, loop_requesting_st.pad_i)

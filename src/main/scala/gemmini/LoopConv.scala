...
class LoopConvLdBias(block_size: Int, coreMaxAddrBits: Int, large_iterator_bitwidth: Int, small_iterator_bitwidth: Int, tiny_iterator_bitwidth: Int, max_acc_addr: Int, acc_w: Int,
  ...
  val och_block_size = Mux(req.num_subarray > 0.U, max_ochs_per_mvin.asUInt() * req.num_subarray, max_ochs_per_mvin.asUInt())
  ...
  when (skip) {
    ...
    when (state === config) {
      ...
      val next_och = floorAdd(och, och_block_size, ochs)
...
/*
  val och_block_size = block_size.U
  when(req.num_subarray > 0.U){
    och_block_size := och_block_size * req.num_subarray
  }

 */
  val och_block_size = Mux(req.num_subarray > 0.U, block_size.asUInt() * req.num_subarray, block_size.asUInt())
...
class LoopConv (block_size: Int, coreMaxAddrBits: Int, rob_size: Int, max_lds: Int, max_exs: Int, max_sts: Int,
  ...
  when(cmd.valid && is_loop_cmd && !loop_being_configured.configured) {
    ...
    switch (cmd.bits.inst.funct) {
      ...
      is (LOOP_CONV_WS_CONFIG_4) {
        ...
        loop_being_configured.inner_bounds.pupad := cmd.bits.rs1(31, 21)
        loop_being_configured.inner_bounds.pdpad := cmd.bits.rs1(20, 10)
        loop_being_configured.outer_bounds.kernel_dilation := cmd.bits.rs1(9, 0)
      ...
      is (LOOP_CONV_WS) {
        ...
        loop_being_configured.num_subarray := cmd.bits.rs1(34, 32)
  ...
  ld_bias.io.req.bits.num_subarray := loop_requesting_ld_bias.num_subarray
  ...
  st.io.req.bits.num_subarray := loop_requesting_st.num_subarray

...
class LoopConvDerivedParams(val large_iterator_bitwidth: Int, val small_iterator_bitwidth: Int, val tiny_iterator_bitwidth: Int) extends Bundle {
  ...
  val krows_times_kcols_times_kchs = UInt(large_iterator_bitwidth.W)
...
class LoopConvLdBias(block_size: Int, coreMaxAddrBits: Int, large_iterator_bitwidth: Int, small_iterator_bitwidth: Int, tiny_iterator_bitwidth: Int, max_acc_addr: Int, acc_w: Int,
  ...
  // Addresses
  val dram_addr = resize(Mux(req.no_bias, 0.U, req.dram_addr + resize(och * (acc_w/8).U, 32)), coreMaxAddrBits)
  val spad_addr = req.addr_start + resize((och / block_size.U) * bias_spad_stride +& b * orows_times_ocols +& orow * ocols +& ocol, log2Up(max_acc_addr))
...
class LoopConvLdInput(block_size: Int, coreMaxAddrBits: Int, large_iterator_bitwidth: Int, small_iterator_bitwidth: Int, tiny_iterator_bitwidth: Int, max_addr: Int, input_w: Int,
  ...
  // Addresses
  val dram_addr = resize(MuxCase(req.dram_addr + resize((((b * in_dim * in_dim +& irow*in_dim +& icol) * in_channels +& ich) * (input_w/8).U).asUInt(), 32), Seq(
    is_zeros -> 0.U,
    req.trans_input_3120 -> (req.dram_addr + resize((((ich * in_dim * in_dim +& irow*in_dim +& icol) * batches +& b) * (input_w/8).U).asUInt(), 32))
  )), coreMaxAddrBits)
...
class LoopConvLdWeight(block_size: Int, coreMaxAddrBits: Int, large_iterator_bitwidth: Int, small_iterator_bitwidth: Int, tiny_iterator_bitwidth: Int, max_addr: Int, input_w: Int,
  ...
  // Addresses
  val dram_addr = resize(MuxCase(req.dram_addr + resize(((krow*kernel_dim*in_channels +& kcol*in_channels +& kch) * out_channels +& och) * (input_w/8).U, 32), Seq(
    req.trans_weight_1203 -> (req.dram_addr + resize(((kch*kernel_dim*kernel_dim +& krow*kernel_dim +& kcol) * out_channels +& och) * (input_w/8).U, 32)),
    req.trans_weight_0132 -> (req.dram_addr + resize(((krow*kernel_dim*out_channels +& kcol*out_channels +& och) * in_channels +& kch) * (input_w/8).U, 32))
  )), coreMaxAddrBits)
...
class LoopConvExecute(block_size: Int, large_iterator_bitwidth: Int, small_iterator_bitwidth: Int, tiny_iterator_bitwidth: Int, max_addr: Int,
  ...
  val c_addr = resize(c_addr_start +&
    (och / block_size.U) * bias_spad_stride +& b * orows_times_ocols +& orow * ocols +& ocol,
    log2Up(max_acc_addr))
  ...
  config_cmd.rs2 := orows_times_ocols << 48
...
class LoopConvSt(block_size: Int, coreMaxAddrBits: Int, large_iterator_bitwidth: Int, small_iterator_bitwidth: Int, tiny_iterator_bitwidth: Int, max_acc_addr: Int, input_w: Int, concurrent_loops: Int, latency: Int)(implicit p: Parameters) extends Module {
  ...
  val dram_addr = resize(Mux(req.trans_output_1203,
    req.dram_addr + resize(((orow*out_dim*batch_size +& ocol*batch_size +& b) * out_channels +& och) * (input_w/8).U, 32),
    req.dram_addr + resize(((b*out_dim*out_dim +& orow*out_dim +& ocol) * out_channels +& och) * (input_w/8).U, 32)),
    coreMaxAddrBits)
  val spad_addr = resize(acc_addr_start +& (och / block_size.U) * bias_spad_stride +& b * orows_times_ocols +& orow * ocols +& ocol, log2Up(max_acc_addr))

  val pool_dram_addr = resize(req.dram_addr + resize(((b * pool_out_dim * pool_out_dim) * out_channels + och) * (input_w/8).U, 32), coreMaxAddrBits)
  val pool_spad_addr = resize(acc_addr_start +& (och / block_size.U) * bias_spad_stride +& b * orows_times_ocols, log2Up(max_acc_addr))
...
class LoopConvState(val block_size: Int, val large_iterator_bitwidth: Int, val small_iterator_bitwidth: Int, val tiny_iterator_bitwidth: Int, val coreMaxAddrBits: Int, val max_addr: Int, val max_acc_addr: Int) extends Bundle {
  ...
  def derived_params(dummy: Int=0): LoopConvDerivedParams = {
    ...
    result.krows_times_kcols_times_kchs := krows * kcols * kchs

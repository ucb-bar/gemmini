...
class Im2Col[T <: Data: Arithmetic](config: GemminiArrayConfig[T]) extends Module {
  ...
  */
  ...
  when(column_counter === block_size.U - 1.U){ //changed for row-wise multiple im2col blocks
  ...
  }.otherwise{window_row_counter := 0.U}
/*
  when(im2col_state =/= im2col_done) {
...
*/
  ...
  }.elsewhen(sram_resp_valid && sram_read_req){ //starting counting & valid requesting
  ...
  when(!sram_read_im2col_fin_d && sram_req_deq_valid_d){
    when((starting_channel_deq + block_size.U) <= req.channel){
      im2col_reg := sram_req_output
    }.otherwise{
      for(i <- 0 until block_size){
        when(i.U < channel_left){
          im2col_reg(i) := sram_req_output(i)
   ...
   */
  sram_read_signals_q.io.enq.valid := sram_resp_valid && sram_read_valid

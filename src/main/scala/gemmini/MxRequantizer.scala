...
class MxRequantizerIO(
  ...
  // mesh
  val requant_data_in = Vec(sp_banks, Flipped(new ScratchpadReadIO(sp_bank_entries, sp_width)))
  val requant_data_out = Vec(sp_banks, Flipped(new ScratchpadWriteIO(sp_bank_entries, sp_width, (sp_width / (aligned_to * 8)) max 1)))

  // sram
  val spad_projected_data = Vec(sp_banks, new ScratchpadWriteIO(sp_bank_entries, sp_width_projected, sp_width_projected/(aligned_to*8) max 1))
  val spad_deprojected_data = Vec(sp_banks, new ScratchpadReadIO(sp_bank_entries, sp_width_projected))
...
class MxRequantizer[T <: Data: Arithmetic](
  ...
  // TODO fix
  // io.scaleMem_write := 0.U.asTypeOf(io.scaleMem_writeq)
  io.lut0_write.ready := 0.B
  io.lut1_write.ready := 0.B
  io.lut2_write.ready := 0.B
  
  // reads
  for (b <- 0 until sp_banks) {
    // default
    io.requant_data_in(b).resp.bits.fromDMA := io.spad_deprojected_data(b).resp.bits.fromDMA
    io.requant_data_in(b).resp.bits.weight_mx_format := io.spad_deprojected_data(b).resp.bits.weight_mx_format
    io.requant_data_in(b).resp.bits.input_mx_format := io.spad_deprojected_data(b).resp.bits.input_mx_format
    
    // ex -> spad 
    io.spad_deprojected_data(b).req <> io.requant_data_in(b).req


    io.requant_data_in(b).resp.valid := io.spad_deprojected_data(b).resp.valid
    io.spad_deprojected_data(b).resp.ready := io.requant_data_in(b).resp.ready

    val proj = io.spad_deprojected_data(b).resp.bits.data
    val spad_data_vec = proj.asTypeOf(Vec(16, UInt(8.W)))

    io.requant_data_in(b).resp.bits.data := VecInit(spad_data_vec.map(_.pad(12))).asUInt
  }

  for (b <- 0 until sp_banks) {
    // default
    io.spad_projected_data(b).addr := io.requant_data_out(b).addr
    io.spad_projected_data(b).mask := VecInit(io.requant_data_out(b).mask.take(16)) // TODO fix properly

    io.requant_data_out(b).ready := io.spad_projected_data(b).ready
    io.spad_projected_data(b).valid := io.requant_data_out(b).valid

    val spad_data_vec = io.requant_data_out(b).data.asTypeOf(Vec(16, UInt(12.W)))

    io.spad_projected_data(b).data := VecInit(spad_data_vec.map(a => a(7, 0))).asUInt
  }

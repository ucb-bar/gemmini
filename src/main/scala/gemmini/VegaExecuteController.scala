...
class VegaExecuteController[T <: Data, U <: Data, V <: Data](xLen: Int, tagWidth: Int, config: GemminiArrayConfig[T, U, V])
  ...
  val vegatileColumns = 1
  val vegameshColumns = 1

  val block_size = meshRows * tileRows //vegameshColumns*vegatileColumns
  //val col_block_size = vegameshColumns*vegatileColumns
  ...
  val DoConfig = functs(0) === CONFIG_VEGA_CMD
  ...
  val mesh_cntl_signals_q = Module(new Queue(new VegaComputeCntlSignals, spad_read_delay+1,
    pipe=true))
  ...
  val mesh = Module(new VegaMeshWithDelays(inputType, spatialArrayOutputType, accType, mesh_tag, dataflow, tree_reduction, tile_latency, mesh_output_delay,
    tileRows, vegatileColumns, meshRows, vegameshColumns, shifter_banks, shifter_banks))
  ...
  val d_row_is_not_all_zeros = d_fire_counter < d_rows //block_size.U - 1.U - d_fire_counter < d_rows //Todo: d_fire_counter_mulpre?
  ...
  val d_mesh_fire_counter = Reg(UInt(log2Up(block_size).W))
  //val new_d = (d_fire_counter === (total_rows-1.U)) || d_fire_counter === 0.U
    ...
    if (ex_read_from_spad) {
      ...
      io.srams.read(i).req.bits.addr := MuxCase(a_address_rs1.sp_row() + a_fire_counter,
        Seq(read_b -> (b_address_rs2.sp_row() + b_fire_counter),
          read_d -> (d_address_rs1.sp_row() + d_fire_counter)))
  ...
  class VegaComputeCntlSignals extends Bundle {
  ...
  mesh_cntl_signals_q.io.enq.bits.d_unpadded_cols := Mux(d_row_is_not_all_zeros, d_rows, 0.U)
  ...
  mesh_cntl_signals_q.io.enq.bits.im2colling := false.B //im2col_wire && im2col_en //im2col_wire
  ...
  when (cntl_valid) {
    ...
    mesh.io.d.valid := cntl.d_fire && (dataD_valid || dataD_reg_valid)

    mesh.io.a.bits := dataA.asTypeOf(Vec(meshRows, Vec(tileRows, inputType)))
    mesh.io.b.bits := dataB.asTypeOf(Vec(vegameshColumns, Vec(vegatileColumns, inputType)))
    mesh.io.d.bits := Mux(d_mesh_fire_counter === 0.U, dataD(block_size.U-1.U).asTypeOf(Vec(vegameshColumns, Vec(vegatileColumns, inputType))), dataD_reg(block_size.U - 1.U - d_mesh_fire_counter).asTypeOf(Vec(vegameshColumns, Vec(vegatileColumns, inputType))))
  ...
  when (cntl_valid && cntl.perform_single_preload) {
    ...
    mesh.io.b.bits := Mux(b_should_be_fed_into_transposer, dataB.asUInt, 0.U).asTypeOf(Vec(vegameshColumns, Vec(vegatileColumns, inputType)))
  }
  ...
  when (cntl_valid && cntl.perform_single_mul) {
    ...
    mesh.io.b.bits := Mux(b_should_be_fed_into_transposer, 0.U, dataB.asUInt).asTypeOf(Vec(vegameshColumns, Vec(vegatileColumns, inputType)))
  ...
  val w_address = mesh.io.resp.bits.tag.addr
  ...
  val w_matrix_cols = mesh.io.resp.bits.tag.rows //cols

  //val output_reg_row = RegInit(Vec(meshRows,mesh.io.resp.bits.data.map(v => VecInit(v.map(e => e.withWidthOf(accType))))))
  val output_reg_row = Reg(Vec(meshRows, Vec(vegameshColumns, accType)))

  val write_this_row = Mux(current_dataflow === Dataflow.WS.id.U, output_counter === w_matrix_rows - 1.U, //output_counter < w_matrix_rows,
  ...
  for (i <- 0 until vega_acc_banks) {
    if (ex_write_to_acc) {
      ...
      io.acc.write(i).bits.data := output_reg_row // VecInit(mesh.io.resp.bits.data.map(v => VecInit(v.map(e => e.withWidthOf(accType)))))
      io.acc.write(i).bits.data(w_matrix_rows-1.U) := mesh.io.resp.bits.data.map(v => VecInit(v.map(e => e.withWidthOf(accType))))(0)
  ...
  when(mesh.io.resp.fire && mesh.io.resp.bits.tag.rob_id.valid) {
    ...
    when (output_counter < w_matrix_rows){
      output_reg_row(output_counter) := mesh.io.resp.bits.data.map(v => VecInit(v.map(e => e.withWidthOf(accType))))(0)
    }

...
class StreamReaderCore[T <: Data, U <: Data, V <: Data](config: GemminiArrayConfig[T, U, V], nXacts: Int, beatBits: Int, maxBytes: Int,
  ...
  lazy val module = new LazyModuleImp(this) with HasCoreParameters with MemoryOpConstants {
    ...
    val io = IO(new Bundle {
      ...
      val alert_cycles = Input(UInt(7.W))
    ...
    when(p_state === p_profile_start){
      ...
      when(tl_profile_end){
        ...
        expected_tl_req := (spad_rows / (2*2*4*2)).asUInt() * io.alert_cycles // scale
      }
    }
    ...
    when(translate_q.io.deq.bits.monitor_conflict && translate_q.io.deq.valid) {
      when(m_state === s_reset) {
        ...
        when(translate_q.io.deq.bits.monitor_conflict_start && translate_q.io.deq.valid) {
          ...
          // set parameters
          alert_cycles := Mux(io.alert_cycles < 8.U, profile_cycle * io.alert_cycles, io.alert_cycles) //alert cycles < 5 -> multiply factor given
          when(io.latency === 0.U) {

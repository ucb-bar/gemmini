...
object GenEvent {
  ...
  def apply(eventName: String, data: UInt, parent: Option[EventAnnotation]): EventAnnotation = {
    var new_id = Wire(UInt(64.W))
    val id_ctr = RegInit(0.U(32.W))
    id_ctr := id_ctr + 1.U 
    new_id := Cat(instance_ctr.asUInt(32.W), id_ctr)
    if (parent.isDefined) {
      printf(cf"{\"id\": \"0x$new_id%x\", \"parents\": \"0x${parent.get.parent}%x\", \"cycle\": \"$id_ctr\", \"event_name\": \"$eventName\", \"data\": \"0x$data%x\"}\n") 
    } else {

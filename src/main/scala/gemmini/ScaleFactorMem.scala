...
class ScalingFactorMem(
  ...
  when(io.write.fire) {
    ...
    val write_bytes_low = io.write.bits.data(bytesPerBank - 1, 0).asTypeOf(bankDataT)
    val write_bytes_high = io.write.bits.data(bytesPerBank * 2 - 1, bytesPerBank).asTypeOf(bankDataT)

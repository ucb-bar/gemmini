...
class QuantLut(
  ...
  val lutCache_act_in  = RegInit(VecInit(Seq.fill(lutConfig(0)._1)(VecInit(Seq.fill(16)(0.U(rdataWidth.W))))))
  val lutCache_weight  = RegInit(VecInit(Seq.fill(lutConfig(1)._1)(VecInit(Seq.fill(16)(0.U(rdataWidth.W))))))
  val lutCache_act_out = RegInit(VecInit(Seq.fill(lutConfig(2)._1)(VecInit(Seq.fill(16)(0.U(rdataWidth.W))))))
  ...
  for (i <- 0 until sp_banks) {
    ...
    when(io.spad_projected_data(i).resp.valid) {
      when(io.read_a) {
        used_lut_act_0 := lutCache_act_in((io.counter_i << 5.U) + (counter_act << 1))
        used_lut_act_1 := lutCache_act_in((io.counter_i << 5.U) + (counter_act << 1) + 1.U)
        for (k <- 0 until 16) { //act data layout is k15a1, k15a0, k14a1, k14a0,...,k0a1,k0a0, each 4 bit, total 32*4
          val used_lut_act = lutCache_act_in(k)
      ...
      when(io.read_d) {
        for (k <- 0 until 32) {
          val used_lut_w = lutCache_weight((io.counter_j << 5.U) + k.U)

...
class Scratchpad[T <: Data: Arithmetic](
  ...
  lazy val module = new LazyModuleImp(this) with HasCoreParameters {
    ...
    val write_dispatch_q = Queue(io.dma.write.req) //from store controller

    write_dispatch_q.ready := false.B

    val write_issue_q = Module(new Queue(new ScratchpadMemWriteRequest(local_addr_t), mem_pipeline+1, pipe=true)) // wait until output of SRAM row came out
    ...
    val reg_valid = WireInit(false.B)
    writer.module.io.req.valid := write_issue_q.io.deq.valid && writeData.valid
    write_issue_q.io.deq.ready := writer.module.io.req.ready && reg_valid//writeData.valid
    writer.module.io.req.bits.vaddr := write_issue_q.io.deq.bits.vaddr //DRAM address from Que
    writer.module.io.req.bits.data := writeData.bits //data to write
    writer.module.io.req.bits.status := write_issue_q.io.deq.bits.status
    //writer: S->D sending commands
    //implement pooling here
    writer.module.io.pool_en := write_issue_q.io.deq.bits.pool_en
    writer.module.io.store_en := write_issue_q.io.deq.bits.store_en
    dontTouch(writer.module.io.store_en)
    writer.module.io.pool_first_row := write_issue_q.io.deq.bits.pool_first_row
    ...
    //Seah: connecting pooling enable to spad pool_enable signal

    val reg_bits = Reg(Vec(16, UInt(8.W)))
    val wire_bits = Wire(Vec(16, UInt(8.W)))//(UInt((spad_w max acc_w).W)))//Wire(Vec(16, UInt(8.W)))
    wire_bits.foreach(_ := 0.U)
    io.busy := writer.module.io.busy || reader.module.io.busy

    {
      val banks = Seq.fill(sp_banks) { Module(new ScratchpadBank(sp_bank_entries, spad_w, mem_pipeline, aligned_to)) } //SRAM bank
      val bank_ios = VecInit(banks.map(_.io))


      // Getting the output of the bank that's about to be issued to the writer
      val bank_issued_io = bank_ios(write_issue_q.io.deq.bits.laddr.sp_bank()) //which bank requested
      //read data from scratchpad at the address laddr

      //Seah: registers for storing values (valid and bits)
      //val reg_valid = WireInit(false.B)
      //val reg_bits = Reg(Vec(16, UInt(8.W)))
      //val wire_bits = Wire(Vec(16, UInt(8.W)))//(UInt((spad_w max acc_w).W)))//Wire(Vec(16, UInt(8.W)))
      //wire_bits.foreach(_ := 0.U)
      when (!write_issue_q.io.deq.bits.laddr.is_acc_addr) {
        //Seah: have to change valid signal for pooling (after all rows are read out)
        when(write_dispatch_q.bits.pool_en){
          reg_valid := bank_issued_io.read.resp.valid && bank_issued_io.read.resp.bits.fromDMA
          when(reg_valid) {
            for (i <- 0 until 15) {
              when(write_dispatch_q.bits.pool_first_row === true.B) {
                reg_bits(i) := bank_issued_io.read.resp.bits.data.asTypeOf(Vec(16, UInt(8.W)))(i)
                wire_bits(i) := bank_issued_io.read.resp.bits.data.asTypeOf(Vec(16, UInt(8.W)))(i)
              }.otherwise {
                reg_bits(i) := maxOf(bank_issued_io.read.resp.bits.data.asTypeOf(Vec(16, UInt(8.W)))(i), reg_bits(i))
                wire_bits(i) := maxOf(bank_issued_io.read.resp.bits.data.asTypeOf(Vec(16, UInt(8.W)))(i), reg_bits(i))
              }
            }
            writeData.valid := write_dispatch_q.bits.store_en
            writeData.bits := wire_bits.asUInt
          }.otherwise{
            writeData.valid := false.B
          }
        }.otherwise{
        ...
        //writeData.valid := bank_issued_io.read.resp.valid && bank_issued_io.read.resp.bits.fromDMA
        //writeData.bits := bank_issued_io.read.resp.bits.data //SRAM bank output data
      }

     // Reading from the SRAM banks
     bank_ios.zipWithIndex.foreach { case (bio, i) => //output of dispatch Que, for loop (bank)
       val ex_read_req = io.read(i).req
       val exread = ex_read_req.valid //is systolic array asking for SRAM data

       // TODO we tie the write dispatch queue's, and write issue queue's, ready and valid signals together here
       val dmawrite = write_dispatch_q.valid && write_issue_q.io.enq.ready && //or DMA want data to write
       ...
       when (exread) {
         ...
         when (bio.read.req.fire()) { //fire: valid and ready both at the same time (SRAM accepts read req)
           write_dispatch_q.ready := true.B //pop out from the dispatch que
           write_issue_q.io.enq.valid := true.B // into the issue que
...

   {
     ...
     val accumulator = Module(new AccumulatorMem(acc_rows, acc_row_t, spad_row_t, mem_pipeline)) //accumulator bank

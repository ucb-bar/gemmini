...
class ZeroWriter[T <: Data, U <: Data, V <: Data, Tag <: Data](config: GemminiArrayConfig[T, U, V], tag_t: Tag)
  ...
  io.resp.bits.laddr := req.bits.laddr + req.bits.block_stride * (col_counter / block_cols.U((col_counter.getWidth+1).W)) // The width expansion here is added solely to satisfy Verilator's linter

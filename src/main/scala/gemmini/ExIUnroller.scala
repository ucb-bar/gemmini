...
class ExIUnroller[T <: Data : Arithmetic, U <: Data, V <: Data](config: GemminiArrayConfig[T, U, V])(implicit p: Parameters)  extends Module {
  ...
  val total_I = q.bits(0).cmd.rs1(63, 48).asUInt() // This is only valid if first_cmd_is_preload === true.B // TODO magic numbers
  ...
  when (q.bits(1).cmd.rs2(31, 0).asTypeOf(local_addr_t).is_garbage()) {

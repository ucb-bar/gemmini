...
class MxExp(inA_exp_width: Int, inW_exp_width: Int, outWidth: Int, elemW: Seq[Int], outTypes: Seq[MxFormats]) extends Module {
  ...
  val in4_w = io.in_w.asTypeOf(Vec(4, UInt((inW_exp_width/4).W)))
  ...
  val sums = elemW.zipWithIndex.map{ case (w, i) =>
    ...
    if (w > inW_exp_width/4) {
      adder.io.b := Mux(io.modeDecoded.weiInputs === 1.U, in1_w(0)(w-1, 0).asSInt, in4_w(i).asSInt.pad(w))
    } else {
      adder.io.b := Mux(io.modeDecoded.weiInputs === 1.U, in1_w(0)(w-1, 0).asSInt, in4_w(i)(w-1, 0).asSInt)
    }
    adder.io.enable := laneMask(i) && io.mask_a(i) && io.mask_w(i)
    adder.io.y
  }
